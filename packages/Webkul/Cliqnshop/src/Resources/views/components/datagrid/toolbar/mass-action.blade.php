            ...
            <div class="flex w-full items-center gap-x-1">
                <x-shop::dropdown position="bottom-left">
                    <x-slot:toggle>
                        ...
                        >
                            <span>
                                @lang('cliqnshop::app.components.datagrid.toolbar.mass-actions.select-action')
                            </span>
                ...
                <div class="ltr:pl-2.5 rtl:pr-2.5">
                    <p class="text-sm font-light text-gray-800">
                        @{{ "@lang('cliqnshop::app.components.datagrid.toolbar.length-of')".replace(':length', massActions.indices.length) }}

                        @{{ "@lang('cliqnshop::app.components.datagrid.toolbar.selected')".replace(':total', available.meta.total) }}
                    </p>
                </div>
            </div>
        </slot>
            ...
            methods: {
                ...
                validateMassAction() {
                    if (! this.massActions.indices.length) {
                        this.$emitter.emit('add-flash', { type: 'warning', message: "@lang('cliqnshop::app.components.datagrid.toolbar.mass-actions.no-records-selected')" });

                        return false;
                    }

                    if (! this.massActions.meta.action) {
                        this.$emitter.emit('add-flash', { type: 'warning', message: "@lang('cliqnshop::app.components.datagrid.toolbar.mass-actions.must-select-a-mass-action')" });
                    ...
                    if (
                        ...
                        this.$emitter.emit('add-flash', { type: 'warning', message: "@lang('cliqnshop::app.components.datagrid.toolbar.mass-actions.must-select-a-mass-action-option')" });
