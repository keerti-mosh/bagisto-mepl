...
<footer class="mt-9 bg-lightOrange max-sm:mt-10">
    <div class="flex justify-between gap-x-6 gap-y-8 p-[60px] max-1060:flex-col-reverse max-md:gap-5 max-md:p-8 max-sm:px-4 max-sm:py-5">
        ...
        >
            <x-slot:header class="rounded-t-lg bg-[#F1EADF] font-medium max-md:p-2.5 max-sm:px-3 max-sm:py-2 max-sm:text-sm">
                @lang('cliqnshop::app.components.layouts.footer.footer-content')
            </x-slot>
        ...
        @if (core()->getConfigData('customer.settings.newsletter.subscription'))
            <div class="grid gap-2.5">
                <p
                    ...
                    aria-level="2"
                >
                    @lang('cliqnshop::app.components.layouts.footer.newsletter-text')
                </p>

                <p class="text-xs">
                    @lang('cliqnshop::app.components.layouts.footer.subscribe-stay-touch')
                </p>

                <div>
                    <x-shop::form
                        :action="route('cliqnshop.subscription.store')"
                    ...
                    >
                        <div class="relative w-full">
                            <x-shop::form.control-group.control
                                ...
                                label="Email"
                                :aria-label="trans('cliqnshop::app.components.layouts.footer.email')"
                            ...
                            >
                                @lang('cliqnshop::app.components.layouts.footer.subscribe')
                            </button>
                        </div>
                    </x-shop::form>
                </div>
    ...
    <div class="flex justify-between bg-[#F1EADF] px-[60px] py-3.5 max-md:justify-center max-sm:px-5">
        ...
        <p class="text-sm text-zinc-600 max-md:text-center">
            @lang('cliqnshop::app.components.layouts.footer.footer-text', ['current_year'=> date('Y') ])
        </p>
