<x-shop::layouts.account>
    <!-- Page Title -->
    <x-slot:title>
        @lang('cliqnshop::app.customers.account.reviews.title')
    </x-slot>
    ...
    @pushOnce('scripts')
                    ...
                    @if (! $reviews->isEmpty())
                        ...
                        <div class="mt-14 grid gap-5 max-1060:grid-cols-[1fr] max-md:mt-5">
                            @foreach($reviews as $review)
                                <a
                                    href="{{ route('cliqnshop.product_or_category.index', $review->product->url_key) }}"
                    ...
                    @else
                        ...
                        <div class="m-auto grid w-full place-content-center items-center justify-items-center py-32 text-center">
                            ...
                            <p
                                ...
                                role="heading"
                            >
                                @lang('cliqnshop::app.customers.account.reviews.empty-review')
                            </p>
                        </div>
                    @endif
                </template>
        ...
        <script type="module">
            app.component("v-product-reviews", {
                ...
                methods: {
                    get() {
                        this.$axios.get("{{ route('cliqnshop.customers.account.reviews.index') }}")
                            .then(response => {
