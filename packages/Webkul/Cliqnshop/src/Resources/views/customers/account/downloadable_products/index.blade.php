<x-shop::layouts.account>
    <!-- Page Title -->
    <x-slot:title>
        @lang('cliqnshop::app.customers.account.downloadable-products.name')
    </x-slot>
    ...
    <div class="mx-4 flex-auto max-md:mx-6 max-sm:mx-4">
        ...
        {!! view_render_event('bagisto.shop.customers.account.downloadable_products.list.before') !!}
            ...
            <!-- For Desktop View -->
        <div class="max-md:hidden">
            <x-shop::datagrid :src="route('cliqnshop.customers.account.downloadable_products.index')" />
        </div>
                ...
                <template #body="{
                    ...
                    <template v-else>
                        <div class="grid gap-4">
                                ...
                                <div class="grid w-full gap-2.5 rounded-md border p-4 transition-all">
                                    <div class="flex justify-between">
                                        <div class="text-sm font-semibold">
                                            <p>@lang('cliqnshop::app.customers.account.downloadable-products.orderId'): #@{{ record.increment_id }}</p>
                            ...
                            <template v-else>
                                @{{ available.records.length }} @lang('cliqnshop::app.customers.account.downloadable-products.records-found')
                            </template>
                        </div>
                    </template>
                </template>
