<x-shop::layouts.account>
    <!-- Page Title -->
    <x-slot:title>
        @lang('cliqnshop::app.customers.account.addresses.index.add-address')
    </x-slot>
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                ...
                <h2 class="text-2xl font-medium max-md:text-xl max-sm:text-base ltr:ml-2.5 md:ltr:ml-0 rtl:mr-2.5 md:rtl:mr-0">
                    @lang('cliqnshop::app.customers.account.addresses.index.title')
                </h2>
            </div>

            <a
                href="{{ route('cliqnshop.customers.account.addresses.create') }}"
                            ...
                            <div class="flex gap-4 max-sm:gap-2.5">
                                @if ($address->default_address)
                                    <div class="label-pending block h-fit w-max px-2.5 py-1 max-md:px-1.5">
                                        @lang('cliqnshop::app.customers.account.addresses.index.default-address')
                                    </div>
                                @endif
                                ...
                                <x-shop::dropdown position="bottom-{{ core()->getCurrentLocale()->direction === 'ltr' ? 'right' : 'left' }}">
                                    ...
                                    <x-slot:menu class="!py-1 max-sm:!py-0">
                                        <x-shop::dropdown.menu.item>
                                            <a href="{{ route('cliqnshop.customers.account.addresses.edit', $address->id) }}">
                                                <p class="w-full">
                                                    @lang('cliqnshop::app.customers.account.addresses.index.edit')
                                                </p>
                                            </a>
                                        </x-shop::dropdown.menu.item>
                                        ...
                                        <x-shop::dropdown.menu.item>
                                            <form
                                                method="POST"
                                                ref="addressDelete"
                                                action="{{ route('cliqnshop.customers.account.addresses.delete', $address->id) }}"
                                            >
                                                @method('DELETE')
                                                @csrf
                                            </form>
                                            ...
                                            >
                                                <p class="w-full">
                                                    @lang('cliqnshop::app.customers.account.addresses.index.delete')
                                                </p>
                                            </a>
                                        </x-shop::dropdown.menu.item>
                                        ...
                                        @if (! $address->default_address)
                                            <x-shop::dropdown.menu.item>
                                                <form
                                                    method="POST"
                                                    ref="setAsDefault"
                                                    action="{{ route('cliqnshop.customers.account.addresses.update.default', $address->id) }}"
                                                >
                                                    @method('PATCH')
                                                    @csrf
                                                ...
                                                >
                                                    <button>
                                                        @lang('cliqnshop::app.customers.account.addresses.index.set-as-default')
                                                    </button>
                                                </a>
                                            </x-shop::dropdown.menu.item>
                                        @endif
        ...
        @else
            ...
            <div class="m-auto grid w-full place-content-center items-center justify-items-center py-32 text-center">
                ...
                <p
                    ...
                    role="heading"
                >
                    @lang('cliqnshop::app.customers.account.addresses.index.empty-address')
                </p>
            </div>
        @endif
    </div>
