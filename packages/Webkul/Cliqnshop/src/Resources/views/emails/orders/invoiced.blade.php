@component('cliqnshop::emails.layout')
    <div style="margin-bottom: 34px;">
        <span style="font-size: 22px;font-weight: 600;color: #121A26">
            @lang('cliqnshop::app.emails.orders.invoiced.title')
        </span> <br>

        <p style="font-size: 16px;color: #5E5E5E;line-height: 24px;">
            @lang('cliqnshop::app.emails.dear', ['customer_name' => $invoice->order->customer_full_name]),👋
        </p>

        <p style="font-size: 16px;color: #5E5E5E;line-height: 24px;">
            @lang('cliqnshop::app.emails.orders.invoiced.greeting', [
                'invoice_id' => $invoice->increment_id,
                'order_id'   => '<a href="' . route('cliqnshop.customers.account.orders.view', $invoice->order_id) . '" style="color: #2969FF;">#' . $invoice->order->increment_id . '</a>',
    ...
    <div style="font-size: 20px;font-weight: 600;color: #121A26">
        @lang('cliqnshop::app.emails.orders.invoiced.summary')
    </div>
        ...
        @if ($invoice->order->shipping_address)
            <div style="line-height: 25px;">
                ...
                <div style="font-size: 16px;font-weight: 400;color: #384860;margin-bottom: 40px;">
                    ...
                    @lang('cliqnshop::app.emails.orders.contact') : {{ $invoice->order->billing_address->phone }}
                </div>
                ...
                <tr style="color: #121A26;border-top: 1px solid #CBD5E1;border-bottom: 1px solid #CBD5E1;">
                    @foreach (['sku', 'name', 'price', 'qty'] as $item)
                        <th style="text-align: left;padding: 15px">
                            @lang('cliqnshop::app.emails.orders.' .$item)
                        </th>
                    @endforeach
                </tr>
            </thead>
    ...
    <div style="display: grid;justify-content: end;font-size: 16px;color: #384860;line-height: 30px;padding-top: 20px;padding-bottom: 20px;">
        ...
        @if ($invoice->discount_amount > 0)
            <div style="display: grid;gap: 100px;grid-template-columns: repeat(2, minmax(0, 1fr));">
                <span>
                    @lang('cliqnshop::app.emails.orders.discount')
                </span>
        ...
        <div style="display: grid;gap: 100px;grid-template-columns: repeat(2, minmax(0, 1fr));font-weight: bold">
            <span>
                @lang('cliqnshop::app.emails.orders.grand-total')
            </span>
