...
>
    <!-- Page Title -->
    <x-slot:title>
		@lang('cliqnshop::app.checkout.success.thanks')
    </x-slot>
	...
	<div class="container mt-8 px-[60px] max-lg:px-8">
		<div class="grid place-items-center gap-y-5 max-md:gap-y-2.5">
			...
			<img
				...
				alt="@lang('cliqnshop::app.checkout.success.thanks')"
				title="@lang('cliqnshop::app.checkout.success.thanks')"
			>
			...
			<p class="text-xl max-md:text-sm">
				@if (auth()->guard('customer')->user())
					@lang('cliqnshop::app.checkout.success.order-id-info', [
						'order_id' => '<a class="text-blue-700" href="'.route('cliqnshop.customers.account.orders.view', $order->id).'">'.$order->increment_id.'</a>'
					])
				@else
					@lang('cliqnshop::app.checkout.success.order-id-info', ['order_id' => $order->increment_id])
				@endif
			</p>
			...
			<p class="text-xl text-zinc-500 max-md:text-center max-md:text-xs">
				...
				@else
					@lang('cliqnshop::app.checkout.success.info')
				@endif
			</p>
