...
>
    <x-slot:header class="font-semibold max-md:py-3 max-md:font-medium max-sm:p-2 max-sm:text-sm">
        @lang('cliqnshop::app.checkout.cart.summary.estimate-shipping.title')
    </x-slot>
        ...
        >
            <form @change="handleSubmit($event, estimateShipping)">
                <p class="mb-4 max-sm:text-sm">
                    @lang('cliqnshop::app.checkout.cart.summary.estimate-shipping.info')
                </p>

                <!-- Country -->
                <x-shop::form.control-group class="!mb-2.5">
                    <x-shop::form.control-group.label class="{{ core()->isCountryRequired() ? 'required' : '' }}">
                        @lang('cliqnshop::app.checkout.cart.summary.estimate-shipping.country')
                    ...
                    <x-shop::form.control-group.control
                        ...
                        :label="trans('cliqnshop::app.checkout.cart.summary.estimate-shipping.country')"
                        :placeholder="trans('cliqnshop::app.checkout.cart.summary.estimate-shipping.country')"
                    >
                        <option value="">
                            @lang('cliqnshop::app.checkout.cart.summary.estimate-shipping.select-country')
                        </option>
                ...
                <x-shop::form.control-group>
                    <x-shop::form.control-group.label class="{{ core()->isStateRequired() ? 'required' : '' }}">
                        @lang('cliqnshop::app.checkout.cart.summary.estimate-shipping.state')
                    </x-shop::form.control-group.label>
                    ...
                    <template v-if="states">
                        <template v-if="haveStates">
                            <x-shop::form.control-group.control
                                ...
                                :label="trans('cliqnshop::app.checkout.cart.summary.estimate-shipping.state')"
                                :placeholder="trans('cliqnshop::app.checkout.cart.summary.estimate-shipping.state')"
                            >
                                <option value="">
                                    @lang('cliqnshop::app.checkout.cart.summary.estimate-shipping.select-state')
                                </option>
                ...
                <x-shop::form.control-group class="!mb-0">
                    <x-shop::form.control-group.label class="{{ core()->isPostCodeRequired() ? 'required' : '' }}">
                        @lang('cliqnshop::app.checkout.cart.summary.estimate-shipping.postcode')
                    </x-shop::form.control-group.label>
                    ...
                    <x-shop::form.control-group.control
                        ...
                        :label="trans('cliqnshop::app.checkout.cart.summary.estimate-shipping.postcode')"
                        :placeholder="trans('cliqnshop::app.checkout.cart.summary.estimate-shipping.postcode')"
                    />
        ...
        app.component('v-estimate-tax-shipping', {
            ...
            methods: {
                ...
                estimateShipping(params, { setErrors }) {
                    ...
                    this.$axios.post('{{ route('cliqnshop.api.checkout.cart.estimate_shipping') }}', params)
                        .then((response) => {
