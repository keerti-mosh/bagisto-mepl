<!-- SEO Meta Content -->
@push('meta')
    <meta name="description" content="@lang('cliqnshop::app.checkout.cart.index.cart')"/>

    <meta name="keywords" content="@lang('cliqnshop::app.checkout.cart.index.cart')"/>
@endPush
...
>
    <!-- Page Title -->
    <x-slot:title>
        @lang('cliqnshop::app.checkout.cart.index.cart')
    </x-slot>
        ...
        <div class="flex w-full justify-between border border-b border-l-0 border-r-0 border-t-0 px-[60px] py-4 max-lg:px-8 max-md:px-4">
            <div class="flex items-center gap-x-14 max-[1180px]:gap-x-9">
                ...
                <a
                    ...
                    aria-label="@lang('cliqnshop::app.checkout.cart.index.bagisto')"
                >
                    <img
            ...
            @guest('customer')
                @include('cliqnshop::checkout.login')
            @endguest
        </div>
    </div>
    ...
    @if (core()->getConfigData('sales.checkout.shopping_cart.cross_sell'))
        ...
        <x-shop::products.carousel
            :title="trans('cliqnshop::app.checkout.cart.index.cross-sell.title')"
            :src="route('cliqnshop.api.checkout.cart.cross-sell.index')"
        >
        </x-shop::products.carousel>
    ...
    @pushOnce('scripts')
            ...
            <div>
                ...
                <template v-else>
                    ...
                    >
                        <div class="flex flex-1 flex-col gap-6 max-md:gap-5">
                            ...
                            <div class="flex items-center justify-between border-b border-zinc-200 pb-2.5 max-md:py-2.5">
                                <div class="flex select-none items-center">
                                    ...
                                    <label
                                        ...
                                        for="select-all"
                                        tabindex="0"
                                        aria-label="@lang('cliqnshop::app.checkout.cart.index.select-all')"
                                    ...
                                    <span
                                        ...
                                        aria-level="2"
                                    >
                                        @{{ "@lang('cliqnshop::app.checkout.cart.index.items-selected')".replace(':count', selectedItemsCount) }}
                                    </span>
                                </div>
                                ...
                                <div v-if="selectedItemsCount">
                                    ...
                                    >
                                        @lang('cliqnshop::app.checkout.cart.index.remove')
                                    </span>
                                    ...
                                    @if (auth()->guard()->check())
                                        ...
                                        >
                                            @lang('cliqnshop::app.checkout.cart.index.move-to-wishlist')
                                        </span>
                                    @endif
                                </div>
                            </div>
                                    ...
                                    <div class="flex gap-x-5">
                                        <div class="mt-11 select-none max-md:mt-9 max-sm:mt-7">
                                            ...
                                            <label
                                                ...
                                                :for="'item_' + item.id"
                                                tabindex="0"
                                                aria-label="@lang('cliqnshop::app.checkout.cart.index.select-cart-item')"
                                        ...
                                        <!-- Cart Item Image -->
                                        <a :href="`{{ route('cliqnshop.product_or_category.index', '') }}/${item.product_url_key}`">
                                        ...
                                        <div class="grid place-content-start gap-y-2.5 max-md:gap-y-0">
                                            ...
                                            >
                                                ...
                                                <div class="">
                                                    ...
                                                    >
                                                        @lang('cliqnshop::app.checkout.cart.index.see-details')
                                                ...
                                                <p class="text-lg font-semibold max-md:text-sm">
                                                    ...
                                                    <template v-else-if="displayTax.prices == 'both'">
                                                        ...
                                                        <span class="text-xs font-normal">
                                                            @lang('cliqnshop.Theme::app.checkout.cart.index.excl-tax')
                                    ...
                                    <div class="text-right max-md:hidden">
                                        ...
                                        <template v-else-if="displayTax.prices == 'both'">
                                            <p class="flex flex-col text-lg font-semibold">
                                                ...
                                                <span class="text-xs font-normal">
                                                    @lang('cliqnshop::app.checkout.cart.index.excl-tax')
                            ...
                            <div class="flex flex-wrap justify-end gap-8 max-md:justify-between max-md:gap-5">
                                ...
                                <a
                                    ...
                                    href="{{ route('cliqnshop.home.index') }}"
                                >
                                    @lang('cliqnshop::app.checkout.cart.index.continue-shopping')
                                </a>
                                ...
                                <x-shop::button
                                    class="secondary-button max-h-14 rounded-2xl max-md:rounded-lg max-md:px-6 max-md:py-3 max-md:text-sm max-sm:py-2"
                                    :title="trans('cliqnshop::app.checkout.cart.index.update-cart')"
                                    ::loading="isStoring"
                                    ::disabled="isStoring"
                                    @click="update()"
                                />
                        ...
                        @include('cliqnshop::checkout.cart.summary')
                    ...
                    >
                        <img
                            ...
                            alt="@lang('cliqnshop::app.checkout.cart.index.empty-product')"
                        />
                        ...
                        <p
                            ...
                            role="heading"
                        >
                            @lang('cliqnshop::app.checkout.cart.index.empty-product')
                        </p>
                    </div>
                </template>
            </div>
        ...
        <script type="module">
            app.component("v-cart", {
                ...
                methods: {
                    getCart() {
                        this.$axios.get('{{ route('cliqnshop.api.checkout.cart.index') }}')
                            .then(response => {
                    ...
                    update() {
                        ...
                        this.$axios.put('{{ route('cliqnshop.api.checkout.cart.update') }}', { qty: this.applied.quantity })
                            .then(response => {
                                this.cart = response.data.data;
                    ...
                    removeItem(itemId) {
                        this.$emitter.emit('open-confirm-modal', {
                            agree: () => {
                                this.$axios.post('{{ route('cliqnshop.api.checkout.cart.destroy') }}', {
                                        '_method': 'DELETE',
                                        'cart_item_id': itemId,
                                    })
                                    .then(response => {
                    ...
                    removeSelectedItems() {
                        this.$emitter.emit('open-confirm-modal', {
                            agree: () => {
                                ...
                                this.$axios.post('{{ route('cliqnshop.api.checkout.cart.destroy_selected') }}', {
                                        '_method': 'DELETE',
                    ...
                    moveToWishlistSelectedItems() {
                        this.$emitter.emit('open-confirm-modal', {
                            agree: () => {
                                ...
                                this.$axios.post('{{ route('cliqnshop.api.checkout.cart.move_to_wishlist') }}', {
                                        'ids': selectedItemsIds,
