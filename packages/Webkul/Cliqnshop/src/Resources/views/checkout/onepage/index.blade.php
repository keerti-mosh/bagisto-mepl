<!-- SEO Meta Content -->
@push('meta')
    <meta name="description" content="@lang('cliqnshop::app.checkout.onepage.index.checkout')"/>

    <meta name="keywords" content="@lang('cliqnshop::app.checkout.onepage.index.checkout')"/>
@endPush
...
>
    <!-- Page Title -->
    <x-slot:title>
        @lang('cliqnshop::app.checkout.onepage.index.checkout')
    </x-slot>
        ...
        <div class="flex w-full justify-between border border-b border-l-0 border-r-0 border-t-0 px-[60px] py-4 max-lg:px-8 max-sm:px-4">
            <div class="flex items-center gap-x-14 max-[1180px]:gap-x-9">
                <a
                    ...
                    aria-label="@lang('cliqnshop::checkout.onepage.index.bagisto')"
                >
                    <img
    ...
    @pushOnce('scripts')
            ...
            <template v-else>
                <div class="grid grid-cols-[1fr_auto] gap-8 max-lg:grid-cols-[1fr] max-md:gap-5">
                    ...
                    <div class="hidden max-md:block">
                        @include('cliqnshop::checkout.onepage.summary')
                    </div>
                        ...
                        >
                            ...
                            <template v-else>
                                <x-shop::button
                                    ...
                                    :title="trans('cliqnshop::app.checkout.onepage.summary.place-order')"
        ...
        <script type="module">
            app.component('v-checkout', {
                ...
                methods: {
                    getCart() {
                        this.$axios.get("{{ route('cliqnshop.checkout.onepage.summary') }}")
                            .then(response => {
                    ...
                    placeOrder() {
                        ...
                        this.$axios.post('{{ route('cliqnshop.checkout.onepage.orders.store') }}')
                            .then(response => {
                                if (response.data.data.redirect) {
                                    window.location.href = response.data.data.redirect_url;
                                } else {
                                    window.location.href = '{{ route('cliqnshop.checkout.onepage.success') }}';
                                }
