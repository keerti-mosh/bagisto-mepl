@if ($product->type == 'downloadable')
    ...
    @if ($product->downloadable_samples->count())
        <div class="sample-list mb-6 mt-8">
            <label class="mb-3 flex font-medium">
                @lang('cliqnshop::app.products.view.type.downloadable.samples')
            </label>
            ...
            <ul>
                @foreach ($product->downloadable_samples as $sample)
                    <li class="mb-2">
                        <a
                            href="{{ route('cliqnshop.downloadable.download_sample', ['type' => 'sample', 'id' => $sample->id]) }}"
                            class="text-blue-700"
                            target="_blank"
                        >
                            {{ $sample->title }}
    ...
    @if ($product->downloadable_links->count())
        <label class="mb-4 mt-8 flex font-medium max-sm:mb-1.5 max-sm:mt-3">
            @lang('cliqnshop::app.products.view.type.downloadable.links')
        </label>
                ...
                <div class="flex select-none items-center gap-x-4">
                    <div class="flex items-center">
                        <v-field
                            ...
                            class="peer hidden"
                            rules="required"
                            label="@lang('cliqnshop::app.products.view.type.downloadable.links')"
                        >
                        </v-field>
                    ...
                    @if (
                        ...
                        <a
                            href="{{ route('cliqnshop.downloadable.download_sample', ['type' => 'link', 'id' => $link->id]) }}"
                            target="_blank"
                            class="text-blue-700 max-sm:text-sm"
                        >
                            @lang('cliqnshop::app.products.view.type.downloadable.sample')
                        </a>
                    @endif
                </div>
            @endforeach
